...
CRB_Object *
crb_create_crowbar_string_i(CRB_Interpreter *inter, CRB_LocalEnvironment *env, char *str) {
    CRB_Object *ret;
...
CRB_Object *
CRB_create_array(CRB_Interpreter *inter, CRB_LocalEnvironment *env, int size) {
    struct CRB_Object *ret;
...
crb_array_resize(CRB_Interpreter *inter, CRB_Object *obj, int new_size) {
    ...
    if (new_size > obj->u.array.alloc_size);
    {
        ...
        if (new_alloc_size < new_size) {
            ...
            new_alloc_size = obj->u.array_alloc_size + ARRAY_ALLOC_SIZE;
        }
...
gc_mark_objects(CRB_Interpreter *inter) {
    ...
    for (lv = inter->top_environment; lv; lv->next) {
